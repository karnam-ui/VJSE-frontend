      ...
      <section className="rounded-[28px] border border-[#1F2937] bg-[#111111] p-8 shadow-xl sm:p-12">
        <div className="grid gap-10 lg:grid-cols-[1.1fr_0.9fr] lg:items-center">
          ...
          <div className="grid gap-4 sm:grid-cols-3">
            ...
            <Card className="rounded-[20px] border border-[#1F2937] bg-[#141414] p-6 text-white shadow-sm">
              <CardTitle className="text-4xl font-semibold">12</CardTitle>
              <CardDescription className="text-sm text-[#9CA3AF]">Domains</CardDescription>
            </Card>
          </div>
        </div>
      </section>
      ...
      <section className="space-y-6 rounded-[28px] border border-[#1F2937] bg-[#111111] p-8 shadow-xl sm:p-10">
        ...
        <div className="flex snap-x snap-mandatory gap-4 overflow-x-auto pb-4">
          {[
            {
              name: "FarmChain",
              description: "Building AgriTech supply-chain tools for fresh produce marketplaces.",
            },
            {
              name: "BrightBridge Labs",
              description: "Using HealthTech analytics to help clinics personalise care delivery.",
            },
            {
              name: "EventHive",
              description: "A student-first event discovery platform for campus communities.",
            },
            {
              name: "EduPulse",
              description: "Delivering adaptive learning journeys for engineering founders.",
            },
          ].map((startup) => (
            <div
              key={startup.name}
              className="min-w-[280px] snap-start rounded-[24px] border border-[#1F2937] bg-[#141414] p-6 shadow-sm"
            >
              <p className="text-sm uppercase tracking-[0.3em] text-[#9CA3AF]">{startup.name}</p>
              <h3 className="mt-4 text-xl font-semibold text-white">{startup.name}</h3>
              <p className="mt-3 text-sm leading-6 text-[#D1D5DB]">{startup.description}</p>
              <div className="mt-5 rounded-3xl bg-[#0F172A] p-4 text-sm text-[#9CA3AF]">
                <p>Domain: {startup.name === "EventHive" ? "Events" : startup.name === "FarmChain" ? "AgriTech" : "HealthTech"}</p>
              </div>
            </div>
          ))}
        </div>
